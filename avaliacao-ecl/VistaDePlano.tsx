...
export function VistaDePlano({ plano, turmaId, nomeProfessor, onVoltar, onPlanoActualizado, onAlteracao, onGuardado }: Props) {
  ...
  /** Requisição já feita que o professor mandou abrir. */
  const [reqAberta, setReqAberta] = useState<string | null>(null);
  ...
  if (modalProximo === 'apos_ficha') {
    ...
    return (
      ...
      /* Uma escolha só, em vez de três perguntas em cadeia.
         Antes era: guião? → requisição? → publicar? — cada uma um modal,
         sem saber quantas faltavam. E perguntava pelo guião mesmo quando
         a ficha já vinha com um da biblioteca. */
      <div style={{ position:'fixed', inset:0, background:'rgba(26,23,20,0.65)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:9999, padding:20 }}>
        <div style={{ background:'#fff', borderRadius:20, padding:26, maxWidth:400, width:'100%' }}>
          <div style={{ fontWeight:700, fontSize:19, marginBottom:4 }}>Ficha guardada</div>
          {nomePrato && (
            <div style={{ fontSize:15, color:'var(--copper)', fontWeight:600, marginBottom:6 }}>
              {nomePrato}
            </div>
          )}

          {/* O que a ficha já traz — para não repetir trabalho feito. */}
          {(ultimaFicha as any)?.textoGuia && (
            <div style={{ background:'var(--sage-pale, #eaf3de)', borderRadius:10,
              padding:'10px 12px', marginBottom:14, fontSize:13.5, color:'var(--sage)',
              fontWeight:600 }}>
              Já traz guião — não precisas de o escrever.
            </div>
          )}

          <div style={{ fontSize:14, color:'rgba(26,23,20,0.55)', marginBottom:14 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
            {!(ultimaFicha as any)?.textoGuia && (
              <button onClick={() => { setModalProximo(null); setModulo('guia'); }}
                style={{ padding:'14px 16px', borderRadius:12, border:'none',
                  background:'var(--sage)', color:'white', fontWeight:700, fontSize:15,
                  cursor:'pointer', fontFamily:'inherit', textAlign:'left' }}>
                Escrever o guião
              </button>
            )}
            <button onClick={() => { setModalProximo(null); setModulo('requisicao'); }}
              style={{ padding:'14px 16px', borderRadius:12,
                border:`1.5px solid var(--copper)`, background:'#fff',
                color:'var(--copper)', fontWeight:700, fontSize:15, cursor:'pointer',
            ...
            {plano.estado !== 'publicado' && (
              <button onClick={() => {
                  ...
                  addOrUpdatePlanoAula(p);
                  onPlanoActualizado(p);
                  setModalProximo(null); setModulo('inicio');
                }}
                style={{ padding:'14px 16px', borderRadius:12,
                  border:'1.5px solid rgba(26,23,20,0.15)', background:'#fff',
                  color:'rgba(26,23,20,0.7)', fontWeight:700, fontSize:15,
                  cursor:'pointer', fontFamily:'inherit', textAlign:'left' }}>
                Publicar a aula para os alunos
              </button>
            )}
            <button onClick={() => { setModalProximo(null); setModulo('inicio'); }}
              style={{ padding:'12px', borderRadius:12, border:'none',
                background:'transparent', color:'rgba(26,23,20,0.5)', fontWeight:600,
                fontSize:14, cursor:'pointer', fontFamily:'inherit', marginTop:2 }}>
              Voltar ao plano
            </button>
          </div>
        </div>
  ...
  if (modulo === 'requisicao') {
    return (
      <div>
            ...
            {todasRequisicoesDoPlano.filter(r => r && r.id).map(r => (
              <div key={r.id} onClick={() => { if (!modoSelecaoReq) return; setReqSelecionadasIds(prev => { const novo = new Set(prev); if (novo.has(r.id)) novo.delete(r.id); else novo.add(r.id); return novo; }); }}
                ...
                <span style={{ fontSize: 13.5, flex: 1 }}>
                  {(r as any).numero && (
                    <b style={{ color: 'var(--sage)' }}>{(r as any).numero} · </b>
                  )}
                  {r.criadaEm ? fmtData(r.criadaEm) : r.id}
                  {' · '}{(r?.linhas || []).length} ingredientes
                </span>
                {!modoSelecaoReq && (
                  <>
                    {/* Abrir a requisição já feita. Antes esta lista só
                        servia para eliminar — o professor voltava ao plano
                        e a aplicação pedia-lhe para gerar tudo de novo. */}
                    <button onClick={(e) => {
                        e.stopPropagation();
                        setFichasParaRequisicao(r.fichasIds || []);
                        setReqAberta(r.id);
                      }}
                      style={{ padding: '6px 13px', borderRadius: 8,
                        border: '1px solid var(--sage)', background: '#fff',
                        color: 'var(--sage)', fontSize: 12.5, fontWeight: 700,
        ...
        {/* `key` força o componente a recomeçar quando se abre outra
            requisição — sem isto ficava com o estado da anterior. */}
        <Requisicao key={reqAberta || 'nova'}
          nomeProfessor={nomeProfessor} planoIdFixo={plano.id} turmaId={turmaId}
