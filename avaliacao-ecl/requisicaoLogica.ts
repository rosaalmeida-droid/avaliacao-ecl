...
// ============================================================
// Produtos parecidos em linhas separadas
// ============================================================
// Uma ficha pede "Ovos M", outra "Ovos L", outra "Ovo inteiro". São o
// mesmo produto no economato, mas ficam em três linhas da requisição —
// e quem vai às compras pede três vezes.
//
// Não se juntam sozinhos: às vezes a distinção é mesmo precisa (manteiga
// com sal e sem sal). O professor é que decide; isto só assinala.

export interface ProdutosParecidos {
  base: string;
  variantes: { produto: string; qt: number; und: string }[];
}

/** Tira o que costuma distinguir variantes do mesmo produto. */
function raizDoProduto(nome: string): string {
  return nome
    .toLowerCase()
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '')   // sem acentos
    .replace(/\s*\([^)]*\)/g, '')                        // (descrições)
    .replace(/\b(m|l|xl|s)\b/g, '')                      // calibres
    .replace(/\b(grande|pequen[oa]|medi[oa]|extra)\b/g, '')
    .replace(/\b(fresc[oa]|congelad[oa]|seco|seca)\b/g, '')
    .replace(/\b(inteir[oa]|partid[oa]|ralad[oa]|picad[oa]|laminad[oa])\b/g, '')
...
/**
 * Agrupa linhas cujo produto parece ser o mesmo.
 * Devolve só os grupos com mais de uma variante.
 */
export function detetarProdutosParecidos(
  linhas: { produto: string; qtEncomenda: number; und: string }[]
): ProdutosParecidos[] {
  const grupos = new Map<string, { produto: string; qt: number; und: string }[]>();

  linhas.forEach(l => {
    if (!l.produto?.trim()) return;
    const raiz = raizDoProduto(l.produto);
    // Raízes muito curtas dariam falsos positivos ("sal" e "salsa").
    if (raiz.length < 4) return;
    const lista = grupos.get(raiz) || [];
    lista.push({ produto: l.produto, qt: l.qtEncomenda, und: l.und });
    grupos.set(raiz, lista);
  });

  const parecidos: ProdutosParecidos[] = [];
  grupos.forEach((variantes, raiz) => {
    // Só interessa quando os NOMES diferem — duas linhas com o mesmo
    // nome exato já foram juntas pela agregação normal.
    const nomesDistintos = new Set(variantes.map(v => v.produto.toLowerCase()));
    if (nomesDistintos.size > 1) {
      parecidos.push({ base: raiz, variantes });
    }
  });
