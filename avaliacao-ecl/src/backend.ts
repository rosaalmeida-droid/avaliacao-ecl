      ...
      <Acc num={1} icon="📄" title="Fichas de produção" desc={feita(1)?resumoFichas:'Seleciona as receitas para esta aula'} status={status(1)} open={secAberta===1} locked={!feita(0)&&secAberta!==1} onToggle={()=>feita(0)&&setSecAberta(1)}>
        {todasFichas.length===0?(
          <div style={{textAlign:'center',padding:'14px 0'}}>
            ...
            <div style={{padding:'12px 14px',background:'var(--sage-pale)',borderRadius:10,fontSize:13,color:'var(--sage)',border:'1px solid rgba(90,122,78,0.2)'}}>
              👆 Vai ao tab <strong>Ficha de Produção</strong> (menu em cima) para criar fichas e depois volta aqui para as associar ao plano.
            </div>
          </div>
        ) : todasFichas.map(f=>(
