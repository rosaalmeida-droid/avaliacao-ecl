...
function agregarIngredientes(fichas: FichaProducao[], paxPorFicha: Record<string, number>): Linha[] {
  ...
  fichas.forEach(f => {
    ...
    const fator = paxPedido / paxBase; // ex: receita p/4, queremos 12 → fator=3

    f.ingredientes.forEach(ing => {
      if (!ing.produto?.trim()) return;

      // Processar ingrediente — converte unidades, exclui água, deriva ingredientes
      const proc = processarIngrediente(ing.produto, ing.qt, ing.un, f.nomePrato);
      if (proc.excluir) return; // água e similares

      // qtKg é a quantidade em unidade base (kg/l/un) para a receita base completa
      const qtReceitaBase = proc.qtKg; // qty para paxBase doses
      const qt1pax = paxBase > 0 ? qtReceitaBase / paxBase : 0;
      const qtEncomenda = qt1pax * paxPedido; // qty para paxPedido doses

      const chave = `${proc.produto.toLowerCase().trim()}__${proc.und}`;
      ...
      if (mapa.has(chave)) {
        ...
        if (proc.avisos.length > 0) l.avisos.push(...proc.avisos);
      } else {
  ...
  // Ordenar: normais primeiro (alfabético), QB no fim
