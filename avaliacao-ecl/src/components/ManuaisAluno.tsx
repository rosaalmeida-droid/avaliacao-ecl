...
- UC LARGAS = MANUAL LONGO: se a UC tem muitos conhecimentos (matérias-primas + cortes + métodos de confeção + conservação, como acontece nas UC de base), NÃO comprimas em 12-15 capítulos — usa 18 a 25. Espremer tudo em poucos capítulos é a principal causa de manuais superficiais. Desdobra: "cortes de legumes" pode dar dois capítulos (cortes regulares com medidas / cortes decorativos e aproveitamento); cada família de confeção pode dar dois (princípios e ciência / execução e aplicações).
- CAPÍTULO OBRIGATÓRIO DE MISE EN PLACE, autónomo (nunca como ponto dentro da introdução): organização do posto; ordem de trabalho; preparação antecipada; medidas e pesagens; gestão do tempo e do lixo.
- NÚMEROS NOS PRÓPRIOS PONTOS DO ÍNDICE (regra dura): os pontos TÊM de trazer os valores, não a promessa deles. É REJEITADO escrever "cozer: temperaturas e tempos" ou "grelhar: técnicas e cuidados". Escreve-se "cozer em fervura a 100°C — massas, batatas, legumes verdes"; "escalfar a 70-80°C sem ferver — ovos, peixe, fruta"; "brasear peças grandes 2-3h a 150-160°C"; "assar em forno a 180-220°C"; "Maillard acima de 140°C — o douramento e o sabor"; "fritar a 175-190°C e ponto de fumo dos óleos"; "confitar a 65-90°C em gordura". O mesmo em cortes: "juliana 3-4 cm x 2 mm", "brunoise 3 mm", "macedónia 5 mm".
- CONTAGEM MÍNIMA (verifica antes de devolver): NENHUM capítulo pode ficar com menos de 6 pontos. Se algum ficar, ou se lhe acrescentam pontos concretos, ou se funde com outro. Capítulos de 3-4 pontos são REJEITADOS.
- CAPÍTULO OBRIGATÓRIO DAS FACAS: se a UC inclui cortes, existe SEMPRE um capítulo dedicado só às FACAS, ANTES dos capítulos de cortes (constituição: lâmina, gume, talão, espiga, cabo; tipos: chef, legumes, desossar, filetar, pão; pega em pinça; postura e pousar na tábua; segurança; transporte e passagem a outra pessoa; afiação com chaira e pedra; lavagem e arrumação). NÃO diluas isto como um ponto solto dentro dos capítulos de cortes.
- PONTOS PRÓPRIOS DE CADA CAPÍTULO (regra dura): é PROIBIDO repetir o mesmo conjunto de pontos em capítulos diferentes. Nada de "temperaturas e tempos; quando usar; exemplos de pratos" copiado em três capítulos seguidos, nem "limpeza e preparação" repetido em todos. Cada capítulo nomeia o que é ESPECÍFICO dele, com os valores concretos: ex.: calor húmido → "fervura a 100°C (cozer); escalfar a 70-80°C sem ferver; estufar tapado com pouco líquido; brasear peças grandes 2-3h"; calor seco → "forno a 180-220°C; reação de Maillard acima de 140°C; grelhar em grelha bem quente; repousar a carne antes de cortar"; calor seco com gordura → "saltear em sertã muito quente, pouca gordura; fritar a 175-190°C; ponto de fumo dos óleos; confitar a 65-90°C".
- MÍNIMO DE 6 PONTOS por capítulo (ideal 8-12). Um capítulo com 4 pontos genéricos é considerado FRACO e tem de ser desdobrado ou aprofundado.
- O CORAÇÃO DO MANUAL SÃO AS REALIZAÇÕES desta UC (o que o aluno EXECUTA). Cada realização gera VÁRIOS capítulos, não um. Ex.: "executar cortes" → um capítulo só para as FACAS (constituição, tipos, pega em pinça, segurança, transporte, afiação) + um para cortes de legumes (juliana, brunoise, macedónia, chiffonade, mirepoix, com medidas) + um para cortes de carnes + um para cortes de peixe. Ex.: "executar técnicas de confeção" → NUNCA num só capítulo: divide por FAMÍLIAS, como nos manuais de referência — calor húmido (cozer, escalfar, estufar, brasear), calor seco (assar, grelhar, gratinar), calor seco com gordura (saltear, fritar, confitar) — cada família com a sua ciência, temperaturas, tempos e aplicações.
- PROPORÇÃO INTERNA (regra dura): os capítulos sobre a EXECUÇÃO (realizações: cortes, confeção, conservação, empratamento) têm de ser MAIS DE METADE do manual. Os capítulos de apoio (matérias-primas, equipamentos) são NECESSÁRIOS mas contidos: no máximo 1/3 do total. NUNCA dês mais capítulos às matérias-primas do que à execução — é o erro mais comum e deixa o manual sem a técnica que a UC exige.
- Inclui sempre, quando a UC os pede: um capítulo de MISE EN PLACE (organização do posto, ordem de trabalho) e um de EQUIPAMENTOS E UTENSÍLIOS (nomeados um a um, com função e manuseamento seguro).
- ESGOTA os temas próprios desta UC:
