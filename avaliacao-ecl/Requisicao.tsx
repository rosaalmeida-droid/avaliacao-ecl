...
import { getPlanosAulaPorTurma, getFichasProducao, addOrUpdateRequisicao, getRequisicaoPorPlano,
  getRequisicoes, numeroParaDocumento, SHEETS_REQUISICAO_URL, getMateriasPrimasCustom, addOrUpdateMateriaPrimaCustom, addAviso, resolverAvisosDoIngrediente, addSugestaoIngrediente } from '../backend';
...
// ═══════════════════════════════════════════════════════════════
export default function Requisicao({ nomeProfessor, planoIdFixo, turmaId = 'CP1', fichasIniciais, onGuardado, onCriarFicha }: {
  nomeProfessor?: string; planoIdFixo?: string; turmaId?: string;
  fichasIniciais?: string[]; onGuardado?: () => void;
  /** Leva o professor a criar uma ficha sem perder a requisição. */
  onCriarFicha?: () => void;
}) {
  ...
  // Sem planoIdFixo, NÃO se agarra um plano qualquer.
  //
  // Antes fazia `planos[0]` — o primeiro da lista. Ao abrir os Orçamentos
  // aparecia logo um plano que o professor não escolheu, e as fichas vinham
  // desse plano. Se ele não tinha fichas, a requisição ficava num beco
  // sem saída: "Este plano não tem fichas de produção associadas".
  //
  // Um orçamento é uma requisição SEM aula. Começa em branco.
  const planoInicial = planoIdFixo
    ? (planos.find(p => p.id === planoIdFixo) || null)
    : null;
  const fichasSelInicial = fichasIniciais?.length
    ? fichasIniciais
    : (planoInicial?.fichasIds || []);
  ...
  /** O calendário só abre se o professor o pedir — num orçamento não serve. */
  const [calendarioAberto, setCalendarioAberto] = useState(!!planoIdFixo);
  ...
  /** Confirmação que aparece e desaparece — numa requisição escolhem-se
   *  várias fichas seguidas, e uma caixa a pedir confirmação em cada
   *  clique daria mais trabalho do que ajuda. */
  const [aviso, setAviso] = useState<string>('');

  function toggleFicha(id: string) {
    const f = todasFichas.find(x => x.id === id);
    const jaEstava = fichasSel.includes(id);
    ...
    if (f) {
      setAviso(jaEstava
        ? `"${f.nomePrato}" saiu ${planoSel ? 'da requisição' : 'do orçamento'}.`
        : `"${f.nomePrato}" juntou-se ${planoSel ? 'à requisição' : 'ao orçamento'}.`);
  ...
  // Produtos que parecem o mesmo em linhas separadas — "Ovos M" e
  // "Ovos L", por exemplo. Não se juntam sozinhos porque às vezes a
  // distinção é mesmo precisa; só se assinala para o professor decidir.
  const parecidos = React.useMemo(
    () => detetarProdutosParecidos(linhasAtivas),
    [linhasAtivas]
  );
      ...
      <div style={{ background: 'var(--requisicao-pale)', borderRadius: 16, padding: 16 }}>
        <div style={{ background: 'var(--requisicao)', borderRadius: 14,
          ...
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 20,
            fontWeight: 700, marginBottom: 4, color: 'white' }}>
            {planoIdFixo ? 'Requisição da aula' : 'Nova requisição'}
          </div>
          <div style={{ fontSize: 13.5, color: 'rgba(255,255,255,0.8)', lineHeight: 1.5 }}>
            {planoIdFixo
              ? 'Escolhe as fichas e define as doses.'
              : 'Escolhe as fichas e define as doses. Ligar a uma aula é opcional — sem aula, fica um orçamento.'}
          </div>
        </div>

        {/* ── LIGAR A UMA AULA — opcional ──
            O calendário estava aberto e parecia o primeiro passo
            obrigatório. Num orçamento não há aula nenhuma a escolher. */}
        {!planoIdFixo && (
          <button onClick={() => setCalendarioAberto(a => !a)} style={{
            width: '100%', padding: '13px 15px', borderRadius: 12, marginBottom: 12,
            border: `1.5px solid ${planoSel ? 'var(--copper)' : 'var(--border)'}`,
            background: planoSel ? 'var(--copper-pale)' : '#fff',
            cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left',
            display: 'flex', alignItems: 'center', gap: 10,
          }}>
            <span style={{ flex: 1 }}>
              <span style={{ display: 'block', fontSize: 14.5, fontWeight: 700,
                color: planoSel ? 'var(--copper)' : 'rgba(26,23,20,0.75)' }}>
                {planoSel ? `Ligada a: ${planoSel.titulo}` : 'Ligar a uma aula (opcional)'}
              </span>
              <span style={{ display: 'block', fontSize: 13, color: 'rgba(26,23,20,0.5)',
                marginTop: 2 }}>
                {planoSel
                  ? 'A requisição fica associada a este plano.'
                  : 'Sem aula, fica guardada como orçamento.'}
              </span>
            </span>
        ...
        {/* ── CALENDÁRIO ── */}
        <div style={{ ...S.card, display: (planoIdFixo || calendarioAberto) ? 'block' : 'none' }}>
        ...
        <div id="req-fichas" style={S.card}>
            ...
            {origemFichas === 'biblioteca' && (
              <div style={{ display: 'flex', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
                <input
                  ...
                  style={{ flex: '1 1 180px', padding: '11px 13px', borderRadius: 10,
                    border: '1px solid var(--border)', fontSize: 14.5,
                    fontFamily: 'inherit' }} />
                {/* Numa requisição pode faltar uma ficha que ainda não
                    existe. Sem isto, o professor tinha de sair, criá-la,
                    e recomeçar a requisição do zero. */}
                {onCriarFicha && (
                  <button onClick={onCriarFicha} style={{
                    padding: '11px 16px', borderRadius: 10, whiteSpace: 'nowrap',
            ...
            {fichasSel.length === 0 && fichasDisp.length > 0 && (
              <div style={{ background: '#fff', border: '1.5px dashed var(--border)',
                borderRadius: 12, padding: '14px 16px', marginBottom: 12,
                fontSize: 14, color: 'rgba(26,23,20,0.55)', textAlign: 'center',
                lineHeight: 1.5 }}>
                Ainda não escolheste nenhuma ficha.<br />
                Toca no quadrado à esquerda de cada uma para a juntar.
              </div>
            )}
            ...
            {/* As escolhidas ficam COLADAS AO TOPO enquanto se desliza
                pela lista. Sem isto, o professor escolhia três fichas,
                deslizava para procurar a quarta, e perdia de vista o que
                já tinha — não havia forma de saber o que estava dentro. */}
            {fichasSel.length > 0 && (
              <div style={{ background: 'var(--copper-pale)',
                border: '2px solid var(--copper)', borderRadius: 12,
                padding: 14, marginBottom: 14,
                position: 'sticky', top: 8, zIndex: 20,
                boxShadow: '0 4px 14px rgba(26,23,20,0.12)' }}>
                <div style={{ display: 'flex', alignItems: 'center',
                ...
                <button onClick={gerarLinhas} style={{
                  width: '100%', padding: 14, borderRadius: 10, border: 'none',
                  background: 'var(--copper)', color: '#fff', fontSize: 15.5,
                  fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit',
                  marginBottom: 12,
                }}>
                  Continuar com {fichasSel.length} ficha{fichasSel.length > 1 ? 's' : ''} →
                </button>
        ...
        {fichasSel.length > 0 && (
          <button onClick={gerarLinhas} style={{
            ...
            background: 'var(--copper)', color: '#fff', fontSize: 17,
            fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit',
          }}>
            Gerar {planoSel ? 'requisição' : 'orçamento'} — {fichasSel.length} ficha{fichasSel.length > 1 ? 's' : ''} · {paxEncTotal} doses
          </button>
        )}
      </div>
    );
    ...
    <div style={{ background: 'var(--requisicao-pale)', borderRadius: 16, padding: 16 }}>
      ...
      <div className="no-print" style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
        <button style={S.btnP} onClick={async () => {
          ...
          //
          // Sem plano também se guarda: é um orçamento. Antes o
          // `if (planoSel)` deitava tudo fora — o professor escolhia
          // fichas, doses e preços, carregava em guardar, e não ficava
          // nada na aplicação.
          const idReq = planoSel
            ? (getRequisicaoPorPlano(planoSel.id)?.id || `req_${planoSel.id}`)
            : `orc_${Date.now()}`;

          // Número próprio para o que é feito fora de um plano. Dentro do
          // plano, identifica-se pelo plano — não precisa de número.
          const jaExiste = getRequisicoes().find(x => x.id === idReq);
          const numero = (jaExiste as any)?.numero
            || numeroParaDocumento(!!planoSel, !planoSel);

          addOrUpdateRequisicao({
            id: idReq,
            planoAulaId: planoSel?.id || '',
            turmaId: planoSel?.turmaId || turmaId,
            dataAula: planoSel?.data || new Date().toISOString().slice(0, 10),
            professor: planoSel?.professor || nomeProfessor || '',
            fichasIds: fichasSel,
            ...
            custoTotal: crTotal, estado: 'enviada',
            criadaEm: new Date().toISOString(), atualizadaEm: new Date().toISOString(),
            // Sem plano é orçamento — para os distinguir na lista.
            ...(numero ? { numero } : {}),
            ...(planoSel ? {} : {
              ehOrcamento: true,
              titulo: familia?.trim() || `Orçamento ${numero}`,
            }),
          } as any);
        ...
        {/* Produtos que parecem o mesmo em linhas separadas. Quem vai às
            compras pede duas vezes sem dar por isso. */}
        {parecidos.length > 0 && (
          <div style={{ background: 'var(--copper-pale)', border: '1.5px solid var(--copper)',
            borderRadius: 12, padding: 15, marginTop: 14 }}>
            <div style={{ fontSize: 14.5, fontWeight: 700, color: 'var(--copper)',
              marginBottom: 4 }}>
              {parecidos.length} produto{parecidos.length > 1 ? 's' : ''} em linhas separadas
            </div>
            <div style={{ fontSize: 13.5, color: 'rgba(26,23,20,0.6)', marginBottom: 11,
              lineHeight: 1.5 }}>
              Vieram de fichas diferentes com nomes diferentes. Se forem o
              mesmo no economato, vale a pena juntar antes de enviar.
            </div>
              ...
              <div key={i} style={{ background: '#fff', borderRadius: 10,
                ...
                {g.variantes.map((v, j) => (
                  <div key={j} style={{ display: 'flex', justifyContent: 'space-between',
                    gap: 10, fontSize: 13.5, padding: '3px 0' }}>
                    <span style={{ color: 'var(--charcoal, #1a1714)' }}>{v.produto}</span>
                    <span style={{ color: 'rgba(26,23,20,0.5)', whiteSpace: 'nowrap' }}>
                      {v.qt.toFixed(3)} {v.und}
                    </span>
                  </div>
                ))}
              </div>
