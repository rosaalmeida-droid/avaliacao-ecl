          ...
          <div key={f.id} className="option-card" onClick={() => {
            ...
            if (mostrarBibliotecaCompleta && planoId) {
              ...
              if (plano && plano.fichasIds.includes(f.id)) {
                alert(`"${f.nomePrato}" já está neste plano.`);
                return;
              }

              // Perguntar antes. Antes associava em silêncio: o professor
              // clicava e nada lhe dizia o que tinha acontecido nem a onde
              // a ficha tinha ido parar.
              const temGuiao = !!(f as any).textoGuia;
              if (!confirm(
                `Juntar "${f.nomePrato}" a este plano de aula?` +
                (temGuiao ? '\n\nA ficha já tem guião — vem com ela.' : '')
              )) return;
              ...
              if (plano) {
                ...
                onGuardado?.();
                alert(
                  `"${f.nomePrato}" juntou-se ao plano.` +
                  (temGuiao
                    ? '\n\nO guião veio com ela — não precisas de o escrever outra vez.'
                    : '')
                );
              }
